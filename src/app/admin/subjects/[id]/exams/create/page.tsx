...
export default function CreateExamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [materialId, setMaterialId] = useState<string>('') // Null jika exam umum
  ...
  // State Dinamis untuk Opsi & Kunci Jawaban (JSONB)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      material_id: materialId ? materialId : null, // Null jika soal exam umum
      type: 'exam', // Memastikan tipenya adalah exam
      question_type: qType,
      question_text: qText,
      options: optionsObj,
    ...
    if (!error) {
      ...
      alert('Gagal membuat soal ujian: ' + error.message)
      setSubmitting(false)
    }
  }
    ...
    <main className="min-h-screen bg-gray-50 p-4 md:p-8 flex justify-center">
      <div className="w-full max-w-3xl bg-white rounded-3xl shadow-sm border border-gray-100 p-6 md:p-10 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between border-b border-gray-100 pb-6">
          <div>
            <span className="text-xs font-semibold text-purple-600 uppercase tracking-wider mb-1 block">Modul Ujian (Exam)</span>
            <h1 className="text-2xl font-bold text-gray-900">Tambah Soal Ujian Baru</h1>
          </div>
          <Link
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Relasi Bab / Material (Opsional untuk Exam) */}
            <div>
          ...
          {/* Teks Soal dengan Preview LaTeX */}
          <div>
          ...
          {/* Bagian Opsi: Pilihan Ganda / Kompleks */}
          ...
          {/* Bagian Opsi: Matriks Benar / Salah */}
