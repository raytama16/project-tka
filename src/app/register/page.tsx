...
export default function RegisterPage() {
  ...
  // Fungsi Register dengan Email & Password (Auto-login & diarahkan ke onboarding data diri)
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // Jika session langsung ada (artinya auto-login aktif tanpa konfirmasi email ketat)
      if (data.session) {
        setMessage('Registrasi berhasil! Mengalihkan ke halaman pengisian data...')
        setTimeout(() => router.push('/onboarding'), 1500)
      } else {
        setMessage('Registrasi berhasil! Silakan cek email Anda untuk verifikasi atau langsung login.')
        setLoading(false)
      }
    }
  }

  // Fungsi Login/Register dengan Google OAuth
  const handleGoogleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        // Setelah sukses login Google, arahkan ke halaman onboarding untuk lengkapi profil
